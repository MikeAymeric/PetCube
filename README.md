...
- **v34 / Companion 0.2.14**: Improved notification sentiment analysis and fixed a stale-icon bug. The sentiment/urgency analyzer now runs on the full text from each plugin (not the 50-char/10-word snippet truncated for the BLE packet), so categorization is no longer biased toward "Routine" by lost context; the BLE packet preview itself is unchanged. The Gmail plugin now also includes the email body (up to 500 chars, HTML stripped if needed) in the analyzed text, not just the sender and subject. On the cube's idle screen, the notification icon and badge — and the long-press battle/dismiss actions — now always reflect the most recently received notification instead of getting stuck on the first one received.
- **v33 / Companion 0.2.13**: Fixed a visual glitch introduced in v32 — on the menu screen, the multiplayer identity tag (when set) overlapped the horizontal divider line drawn below the creature sprite, with the divider striking through the bottom of the tag text. The tag is now drawn flush against the sprite, just above the divider.
- **v32 / Companion 0.2.12**: UI tweaks following user feedback on v30/v31. The clock footer was reverted to its smaller, centered v29 layout (no sun/moon icon), keeping the white text color introduced in v30. The Bluetooth icon was resized to 24x24, between the original 16x16 and the 32x32 used in v30/v31. The multiplayer identity tag was removed from the status screen (where it could get clipped by the round display's edge) and is now shown centered below the creature sprite on the menu screen.
