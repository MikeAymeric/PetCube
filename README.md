...
- **Companion 0.2.9**: The clock sync over BLE (introduced in v27) now also happens when checking the firmware version or refreshing achievements via BLE, not just when sending a notification — so the cube's clock stays accurate even if no notifications are sent for a while.
